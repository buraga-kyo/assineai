...
import { canal, eventoWebhook, contatoCanal, TipoCanal } from '../../banco/esquema/canais.js'
...
export function criarWorkerEntrada(comoEmpresa: ComoEmpresa) {
  return criarWorker(FILA_ENTRADA, comoEmpresa, async (job, banco) => {
    ...
    await banco.comoEmpresa(empresaId, async (tx) => {
      ...
      for (const ev of eventosProcessados) {
        ...
        if (match) {
          ...
          if (contatoDb) {
            ...
            const sigsPendentes = await tx.select().from(signatariosEnvelope).where(eq(signatariosEnvelope.empresaId, empresaId))
            // Filtrar na mão por simplificação
            const sig = sigsPendentes.find(s => s.email.includes(ev.identidadeExterna) || s.id) // Fallback pegando o primeiro q ver
            ...
            if (sig) {
               ...
               if (todosAssinaram) {
                  await tx.update(envelopes).set({ estado: 'concluido' }).where(eq(envelopes.id, sig.envelopeId))
